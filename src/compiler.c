 ...
 * @file compiler.c
 * @author Devin Arena
 * @brief Compiler for Palladium implementation.
 * @since 5/20/2022
 **/
...
static void declareVariable() {
  ...
  for (int i = current->localCount - 1; i >= 0; i--) {
    ...
    if (identifiersEqual(name, &local->name)) {
      errorAtCurrent("Redefinition of variable in scope.");
    }
  }
...
static void this_(bool canAssign) {
  if (currentClass == NULL) {
    errorAtCurrent("Cannot use 'this' outside of a class.");
    return;
  }
  variable(false);
}
  ...
  if (!check(TOKEN_RIGHT_PAREN)) {
    do {
      ...
      if (current->function->arity > 255) {
        errorAtCurrent("Too many parameters (MAX=255).");
      }
...
static void classDeclaration() {
  ...
  if (match(TOKEN_LESS)) {
    ...
    if (identifiersEqual(&parser.previous, &className)) {
      errorAtCurrent("A class cannot inherit from itself.");
    }
...
static void returnStatement() {
  ...
  if (match(TOKEN_SEMICOLON)) {
    ...
    if (current->type == TYPE_INITIALIZER) {
      errorAtCurrent("Cannot return a value from an initializer.");
    }
