use crate::{
    expression::expression::{Expression, Visited, Visitor},
    token::TokenType,
    value::Value,
};
    ...
    fn visit_binary(&mut self, binary: &Expression) -> Value {
        match binary {
            Expression::Binary(left, token_type, right) => {
                ...
                match token_type {
                    TokenType::PLUS => match left_value {
                        Value::Integer(left_integer) => match right_value {
                        ...
                        Value::Float(left_float) => match right_value {
                            Value::Integer(right_integer) => {
                                Value::Float(left_float + right_integer as f32)
                            }
                            Value::Float(right_float) => Value::Float(left_float + right_float),
                            _ => {
                                self.runtime_error("Operation undefined for types");
                                Value::Integer(0)
                            }
                    ...
                    TokenType::MINUS => match left_value {
                        Value::Integer(left_integer) => match right_value {
                            Value::Integer(right_integer) => {
                                Value::Integer(left_integer - right_integer)
                            }
                    ...
                    TokenType::STAR => match left_value {
                        Value::Integer(left_integer) => match right_value {
                            Value::Integer(right_integer) => {
                                Value::Integer(left_integer * right_integer)
                            }
                    ...
                    TokenType::SLASH => match left_value {
                        Value::Integer(left_integer) => match right_value {
                            Value::Integer(right_integer) => {
                                Value::Integer(left_integer / right_integer)
                            }
